...
export class ServerSynchronizerService {
  ...
  private readonly baseUrl = `${environment.apiUrl}/governance/sync`;
  ...
  commandRequested<TRequest>(action: string, payload: TRequest): Observable<void> {
    return this.http.post<SyncPayload>(`${this.baseUrl}/${action}`, { payload })
    .pipe(
      map((syncPayload: SyncPayload) => this.sync(syncPayload)),
      catchError((err) => of().pipe(
        tap(() => this.snackBar.open(err, "SERVER ERROR", { duration: 2000 }))
      ))
    );
  }

  private sync(syncPayload: SyncPayload): void {
